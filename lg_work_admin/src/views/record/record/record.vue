			...
			<el-tabs tab-position="left" style="height: 500px" class="dialog-tabs">
				<el-tab-pane class="course">
					...
					<div class="radio-button">
						<el-button v-for="item in courses.data" :key="item.id" type="primary" plain @click="selectCourse(item.id!)"> {{ item.name }}</el-button>
					</div>
            ...
            >
				...
				<el-tab-pane class="start">
					...
					<el-radio-group v-model="form.start_mould" @change="preview">
						<el-radio v-for="item in moulds.start_moulds" :key="item.id" :label="item.content" size="large">{{ item.content }}</el-radio>
					</el-radio-group>
				</el-tab-pane>
				<el-tab-pane>
					<template #label>
				...
				<el-tab-pane>
					...
					<el-input v-model="form.behavior_mould" :autosize="{ minRows: 10, maxRows: 15 }" type="textarea" placeholder="请填写课堂表现" @change="preview" />
				</el-tab-pane>
				<el-tab-pane>
					<template #label>
						<el-badge :value="form.specific_mould && form.specific_mould.length > 0 ? '+1' : ''"><span>细节描述</span></el-badge>
					</template>
					<el-input v-model="form.specific_mould" :autosize="{ minRows: 10, maxRows: 15 }" type="textarea" placeholder="请填写细节描述" @change="preview" />
				</el-tab-pane>
				<el-tab-pane class="end">
					...
					<el-scrollbar height="500px">
						<el-radio-group v-model="form.end_mould" @change="preview">
							<el-radio v-for="item in moulds.end_moulds" :key="item.id" :label="item.content" size="large">{{ item.content }}</el-radio>
						</el-radio-group>
					</el-scrollbar>
				</el-tab-pane>
			</el-tabs>
			<el-descriptions title="效果预览" :column="1" class="dialog-description">
				<el-descriptions-item label="姓名">{{ select_student.name }}</el-descriptions-item>
				<el-descriptions-item label="课程主题">{{ form.courseName }}</el-descriptions-item>
